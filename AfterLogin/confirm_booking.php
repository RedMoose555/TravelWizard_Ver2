...
function validateBookingForm($post) {
    $errors = [];

    if (!preg_match('/^\d{16}$/', $post['card_number'] ?? '')) {
        $errors[] = "Card number must be exactly 16 digits.";
    }

    if (!preg_match('/^\d{3}$/', $post['cvc'] ?? '')) {
        $errors[] = "CVC must be exactly 3 digits.";
    }

    if (!preg_match('/^(0[1-9]|1[0-2])\/\d{2}$/', $post['expiry_date'] ?? '')) {
        $errors[] = "Expiry date must be in MM/YY format.";
    }

    if (!preg_match('/^(0[1-9]|[12][0-9]|3[01])\/(0[1-9]|1[012])\/\d{4}$/', $post['dob'] ?? '')) {
        $errors[] = "Date of birth must be in DD/MM/YYYY format.";
    }

    if (!preg_match('/^\d+$/', $post['passport_number'] ?? '')) {
        $errors[] = "Passport number must be numeric.";
    }

    if (!preg_match('/^\d{7,}$/', $post['emergency_phone'] ?? '')) {
        $errors[] = "Emergency phone must be at least 7 digits and contain only numbers.";
    }
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['final_confirm'])) {
    ...
    $errors = validateBookingForm($_POST);
    if (!empty($errors)) {
        // Show errors and link to go back
    ...
    if (!$package_id || !$departureFlight || !$returnFlight) {
        echo "<p style='color:red;'>Booking details missing.</p>";
        exit();
    }
